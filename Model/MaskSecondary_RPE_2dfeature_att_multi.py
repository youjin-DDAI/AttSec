...
def scaled_dot_product_attention(q, k, v, position_bias, mask, rp_mask):
    ...
    attention = logits
    attention_weights = F.softmax(logits, dim=-1)#nn.Softmax(dim=-1)(logits)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        batch_size = torch.tensor(query.size(0))
        ...
        position_bias, rp_mask = self.compute_bias(torch.tensor(q.size(-2)), torch.tensor(k.size(-2)))
...
class MASKSecondary(nn.Module):
    def __init__(self, dropout, dssp_dim):
        ...
        self.relu = nn.ReLU()
        self.elu = nn.ELU()
        seg_dropout = 0.0  # nn.Dropout(dropout)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, mask):
        ...
        h = self.tanh(attention)

        h1 = h2 = h3=self.first(h)

        for _, (sd1, sd2, sd3) in enumerate(zip(self.segment_detector_1, self.segment_detector_2, self.segment_detector_3)):
            h1 = sd1(h1)
            h2 = sd2(h2)
            h3 = sd3(h3)
